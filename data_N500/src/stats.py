"""
Statistical readouts for the experiment.

  * two_proportion_readout — headline frequentist result (lift, Wald CI,
    pooled two-sided z-test)
  * mean_diff_readout      — difference in means with Welch SE; used for
    CUPED-adjusted (non-binary) outcomes
  * segment_readout        — per-segment lifts, always exploratory
  * bayesian_readout       — Beta-Binomial posterior companion

Run the full readout on a cohort:
...
def normalize_cohort(df: pd.DataFrame) -> pd.DataFrame:
    """Tolerate historical column names from earlier pipeline versions."""
...
def mean_diff_readout(y_t: pd.Series, y_c: pd.Series,
                      alpha: float = DEFAULT_ALPHA) -> dict:
    """Difference in means with Welch SE — works for adjusted outcomes."""
...
def segment_readout(df: pd.DataFrame, segment_col: str = "archetype",
    ...
    """Per-segment lifts with CIs. Exploratory by construction: every extra
    segment is another chance at a spurious p < 0.05."""
    rows = []
...
def print_headline(trt: pd.Series, ctrl: pd.Series,
    ...
    print()
    if r["significant"]:
        print(f"VERDICT: Significant at alpha = {alpha}.")
    else:
        print("VERDICT: Not significant. We cannot distinguish this from no effect.")
        print("  The confidence interval includes zero, so a true effect of exactly")
        print("  nothing remains entirely consistent with this data.")
    return r
...
def main() -> None:
    ...
    if "archetype" in cohort.columns:
        print()
        print("By segment (exploratory -- do not over-read)")
        print("=" * 55)
